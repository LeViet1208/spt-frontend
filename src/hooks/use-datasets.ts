...
import { datasetAPI, type Dataset } from "@/lib/api/dataset"
...
export function useDatasets() {
  ...
  const createDataset = useCallback(
    async (request: CreateDatasetRequest, onProgress?: (progress: CreateDatasetProgress) => void) => {
      try {
        ...
        if (!transactionResponse.success) {
          return {
            success: false,
            error: transactionResponse.error || "Failed to upload transaction file",
          }
        }

        // Update dataset status
        setDatasets((prev) =>
          prev.map((d) => (d.id === datasetId ? { ...d, importStatus: "importing_product_lookup" as const } : d)),
        )
        ...
        if (!productResponse.success) {
          return {
            success: false,
            error: productResponse.error || "Failed to upload product lookup file",
          }
        }

        // Update dataset status
        setDatasets((prev) =>
          prev.map((d) => (d.id === datasetId ? { ...d, importStatus: "importing_causal_lookup" as const } : d)),
        )
        ...
        if (!causalResponse.success) {
          return {
            success: false,
            error: causalResponse.error || "Failed to upload causal lookup file",
          }
        }

        // Final update - mark as completed
        setDatasets((prev) =>
          prev.map((d) => (d.id === datasetId ? { ...d, importStatus: "import_completed" as const } : d)),
        )
  ...
  const deleteDataset = useCallback(async (id: number) => {
    try {
      ...
      if (response.success) {
        ...
        return { success: false, error: response.error || "Failed to delete dataset" }
      }
    } catch (err) {
